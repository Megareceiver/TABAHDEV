...
function r_f4AnggotaTimWilayah(packet, referencesId) {
	...
	$('main.parent').animate({'opacity': '0.6'},'fast','linear', function(){
		...
		data = p_getData('f4', 'anggotaTimWilayah', "idTimWilayah = '" + packet + "'");
		data = data.feedData;
		optionBatch = r_f4OptionList("deleteOnly"); 
					...
					'<div class="row default">' +
						'<div class="col-md-12">' +
							'<div class="select-box">' +
								'<select>' +
									'<option value="">Pilih Anggota</value>' +
								'</select>' +
							'</div>' +
						'</div>' +
					'</div>' +
...
function r_f4GrupPersyaratan() {
	...
	$('main.parent').animate({'opacity': '0.6'},'fast','linear', function(){
		...
		content = '';
		data    = [
			{'noreg': '1', 'caption': 'Verifikasi dokumen'},
		];
		counter = 1;
